...
{
    static public Dictionary<string, string> ToCS(Dictionary<string, Table> tables, string nameSpace = "", string baseClass = "")
    {
        ...
        if (tables != null)
        {
            foreach (var table in tables)
            {
                ...
                if (table.Value.datas.Count > 1)
                {
                    ...
                    cs[table.Key] += space + "    }\n";
                    cs[table.Key] += "\n";
                    cs[table.Key] += space + "    public OrderedDictionary<"
                        + keyType
                        + ", _> datas = new OrderedDictionary<"
                        + keyType
                        + ", _>();\n";
